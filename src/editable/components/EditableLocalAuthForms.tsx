...
const inputClass = 'h-[3.25rem] border border-black/25 bg-white px-4 text-base font-bold text-black outline-none transition placeholder:text-black/38 focus:border-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-[3.25rem] items-center justify-center border border-[var(--slot4-dark-bg)] bg-[var(--slot4-dark-bg)] px-6 text-xs font-black uppercase tracking-[0.2em] text-white transition hover:bg-[var(--slot4-accent)] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      <label className="grid gap-2 text-xs font-black uppercase tracking-[.16em] text-black/65">Email address<input className={inputClass} type="email" placeholder="name@company.com" value={email} onChange={(event) => setEmail(event.target.value)} required /></label>
      <label className="grid gap-2 text-xs font-black uppercase tracking-[.16em] text-black/65">Password<input className={inputClass} type="password" placeholder="Your password" value={password} onChange={(event) => setPassword(event.target.value)} required /></label>
...
export function EditableLocalSignupForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      <label className="grid gap-2 text-xs font-black uppercase tracking-[.16em] text-black/65">Full name<input className={inputClass} placeholder="Campaign owner" value={name} onChange={(event) => setName(event.target.value)} required /></label>
      <label className="grid gap-2 text-xs font-black uppercase tracking-[.16em] text-black/65">Email address<input className={inputClass} type="email" placeholder="name@company.com" value={email} onChange={(event) => setEmail(event.target.value)} required /></label>
      <label className="grid gap-2 text-xs font-black uppercase tracking-[.16em] text-black/65">Password<input className={inputClass} type="password" placeholder="At least 4 characters" value={password} onChange={(event) => setPassword(event.target.value)} required /></label>
      {message ? <p className={`border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-800 bg-emerald-50 text-emerald-800' : 'border-red-700 bg-red-50 text-red-700'}`}>{message}</p> : null}
