...
import { fetchSiteFeed, type SiteFeed, type SiteFeedPagination, type SitePost } from '@/lib/site-connector'
...
const normalizeTaskType = (value: string) => value.trim().toLowerCase().replace(/[\s_-]+/g, '')
...
const getEditablePostType = (post: SitePost) => {
  const content = getContent(post)
  const explicit = asText(content.type) || asText(content.task) || asText(content.contentType)
  if (explicit) return explicit
  const tag = Array.isArray(post.tags) ? post.tags.find((item) => mediaDistributionAliases.has(normalizeTaskType(String(item)))) : ''
  return tag || ''
}
...
const isMediaDistributionPost = (post: SitePost) => {
  ...
  const postType = getEditablePostType(post)
  if (!postType) return true
  return mediaDistributionAliases.has(normalizeTaskType(postType))
}

const mediaFallbackPagination = (page: number, limit: number, total: number): SiteFeedPagination => {
...
async function fetchEditableMediaDistributionFallback(page: number, limit: number, category: string) {
  const offset = (page - 1) * limit
  const categoryFilter = category === 'all' ? '' : category
  const feeds = await Promise.all(
    [
      fetchSiteFeed<SitePost>(100, { fresh: true, timeoutMs: 5000 }).catch(() => null),
      fetchSiteFeed<SitePost>(100, { task: 'mediaDistribution', fresh: true, timeoutMs: 5000 }).catch(() => null),
      fetchSiteFeed<SitePost>(100, { task: 'mediadistribution', fresh: true, timeoutMs: 5000 }).catch(() => null),
      fetchSiteFeed<SitePost>(100, { task: 'media', fresh: true, timeoutMs: 5000 }).catch(() => null),
      fetchSiteFeed<SitePost>(100, { task: 'press-release', fresh: true, timeoutMs: 5000 }).catch(() => null),
    ]
  )
  const merged = Array.from(
    new Map(
      feeds
        .flatMap((feed: SiteFeed<SitePost> | null) => feed?.posts || [])
        .filter(isMediaDistributionPost)
        .filter((post) => {
          if (!categoryFilter) return true
          const postCategory = getCategory(post, '')
          return normalizeCategory(postCategory) === categoryFilter
        })
        .map((post) => [post.slug || post.id || post.title, post])
    ).values()
  )
  return {
    posts: merged.slice(offset, offset + limit),
    pagination: mediaFallbackPagination(page, limit, merged.length),
  }
}
...
export async function EditableTaskArchiveRoute({
  ...
  const initial = await fetchPaginatedTaskPosts(task, { page, limit: 24, category })
  const data = task === 'mediaDistribution' && !initial.posts.length
    ? await fetchEditableMediaDistributionFallback(page, 24, category)
    : initial
  const resolvedBasePath = task === 'mediaDistribution' ? '/media-distribution' : basePath || taskConfig?.route || `/${task}`
  return <TaskArchiveView task={task} posts={data.posts} pagination={data.pagination} category={category} basePath={resolvedBasePath} />
}
      ...
      <main className="min-h-screen bg-[#f7f4ef] text-[#111]">
        <section className="border-b border-black/10 bg-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[var(--slot4-accent)]">Distribution archive</p>
              <h1 className="mt-3 text-5xl font-black leading-none tracking-[-0.055em] sm:text-6xl lg:text-7xl">
            ...
            <p className="max-w-md border-l-4 border-[var(--slot4-accent)] pl-5 text-sm font-bold leading-7 text-black/65">
              Press releases, brand announcements, publisher-ready campaign cards, and media outreach content organized for fast discovery.
            </p>
          </div>
        </section>

        <section className="border-b border-black bg-[var(--slot4-dark-bg)] text-white">
          <div className="mx-auto flex max-w-[var(--editable-container)] gap-7 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.16em] sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'text-[#f4b63d]' : 'hover:text-[#f4b63d]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'text-[#f4b63d]' : 'whitespace-nowrap hover:text-[#f4b63d]'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[var(--editable-container)] border-x border-black bg-white lg:grid-cols-[1.75fr_0.75fr]">
            ...
            <div className="grid">
              <div className="border-b border-black bg-[var(--slot4-accent)] p-6 text-white">
                <p className="text-xs font-black uppercase tracking-[0.24em]">Priority campaigns</p>
                <p className="mt-3 text-3xl font-black leading-tight tracking-[-.04em]">Announcements ready for wider media pickup.</p>
              </div>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] border-x border-black bg-[#f7f4ef] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b-4 border-black pb-4">
            <h2 className="text-4xl font-black tracking-[-0.04em] sm:text-5xl">Campaign cards</h2>
          ...
          {remaining.length ? (
              ...
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group border-b border-r border-black bg-white">
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="mt-4 line-clamp-2 text-2xl font-black leading-[1.05] tracking-[-.04em]">{post.title}</h3>
                    <p className="mt-4 line-clamp-3 text-sm leading-6 text-black/60">{getSummary(post)}</p>
                    <div className="mt-5 grid grid-cols-3 gap-2 text-[10px] font-black uppercase tracking-[.1em] text-black/55">
                      <span className="bg-[var(--slot4-accent-soft)] px-2 py-2 text-[var(--slot4-accent)]">Live</span>
                      <span className="bg-[#fff4d8] px-2 py-2">Reach {(index + 4) * 16}K</span>
                      <span className="bg-[#edf3f7] px-2 py-2">PR ready</span>
                    </div>
                  </div>
                </Link>
              ))}
            ...
            <div className="border border-dashed border-black bg-white p-12 text-center">
              ...
              <h2 className="mt-4 text-3xl font-black tracking-[-.04em]">No campaigns found</h2>
              <p className="mt-2 text-sm text-black/60">Try another category or publish a new media distribution campaign.</p>
            </div>
          ) : null}
